              ...
              >
                ...
                >
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    <span className="shrink-0 inline-flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 dark:bg-[var(--ds-surface-overlay,#2B2C2F)] text-xs font-semibold text-slate-700 dark:text-[var(--ds-text,#CECFD2)]">
                      {getInitials(selectedInstitution?.acronym)}
                    </span>
                    <span className="text-sm font-medium text-slate-800 dark:text-[var(--ds-text,#CECFD2)] truncate max-w-[220px]">
...
const AppRow = ({
  ...
  const icon =
    !item.icon ? (
      ...
      <img src={item.icon} className="h-6 w-6 object-contain" alt="" />
    ) : (
